...
/* ---- FIRST SEQUENCE, START @ LEFT SIDE OF PARKZONE FACING MATCHLOAD ---- */
    ...
    chassis.pid_wait_quick_chain();
    chassis.pid_drive_set(18_in, 127); // drive into matchload
    chassis.pid_wait_quick_chain();
    ...
    chassis.pid_drive_set(130_in, 90); // go line up infront of alleyway
    chassis.pid_wait_quick_chain();
    pros::delay(100);
    chassis.pid_turn_set(-180_deg, 90); // turn to face the exit alleyway
    chassis.pid_wait_quick_chain();
    chassis.pid_drive_set(16_in, 127); // exit alleyway
    ...
    chassis.pid_drive_set(30_in, 127); // back away from goal
    chassis.pid_wait_quick_chain();
    matchload.set(true); // lower matchload
    intake.move(127); // start intake
    chassis.pid_drive_set(20_in, 127); // complete action towards the goal
    chassis.pid_wait_quick_chain();
    pros::delay(1000); // give time to intake balls
    chassis.pid_wait_quick_chain();
